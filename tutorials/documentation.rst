...
Для того, что бы в системе Read The Docs собиралась документация из репозитория необходимо в самом репозитории 
...
В качестве примера орагинзации структуры можно заглянуть в `open-source проект <https://github.com/aimclub/GEFEST>`_.
...
Создать начальную структуру документации внутри проекта можно **2 способами**:
   ...
   1. С помощью библиотеки sphinx:
     ...
     **Шаг 2**. `Создание структуры документаци с помощью команды <быстрой настройки https://www.sphinx-doc.org/en/master/usage/quickstart.html>`_
...
Описание некоторых из них:
   ...
   - ``sphinx_rtd_theme`` - устанавливает тему отображения документации (альтерантива - ``alabaster``)
   - ``sphinx.ext.autodoc`` - позволяет формировать документацию из докстрингов 
   - ``sphinx.ext.napoleon`` - добавляет в окружение проекта пакет *napoleon*, способный расширать функционал отображения документации
...
В данном разделе указываются дополнительные парметры для расширений ``extensions``, которые описаны в ``3.1 General configuration``.
...
На сайте дкументации в Read The Docs этот файл помимо начальной страницы, которая выглядит `вот так <https://github.com/aimclub/GEFEST/blob/main/docs/source/index.rst>`_, 
...
В время создания таких файлов с контентом удобно использовать встроенные в IDE *preview*, для этого нужно установить 
...
Оформление страниц документации на сайте Read The Docs, содержащей информацию о методах и классах из докстрингс осуществляется так же через
файлы с расширением ``.rst``. `Пример оформления файла с использованием докстрингс <https://github.com/aimclub/GEFEST/blob/main/docs/source/components/geometry.rst>`_.
...
`Файл с кодом (контентом в докстрингс) <https://github.com/aimclub/GEFEST/blob/main/gefest/core/geometry/geometry_2d.py>`_.
...
одного конкретного класса во всем файле ``.py``. В случае, когда в документацию необъодимо поместить все докстринги из файла с кодом, нужно использовать команду 
...
В тот момент, когда происходит отправка очередного коммита с докуменацией в Pull Request на GitHub, документация начинает собираться автоматически:
